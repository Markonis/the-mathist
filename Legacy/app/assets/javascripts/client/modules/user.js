...
app.modules.user = {
  init: function() {
    ...
    module.choose_account_dialog = $('#choose-account-dialog');
  ...
  actions: {
    providers_loaded: function() {
      var module = app.modules.user;
      module.functions.setup_google_button();
      app.notify({
        type: "open_dialog_tap",
        data: {
          dialog: module.choose_account_dialog
        }
      });
    },

    account_chosen: function(event) {
      var module = app.modules.user;
      module.session = event.data.session;
      ...
      $.ajax({
        type: 'POST',
        url: "client/new_session",
        data: JSON.stringify(event.data.session),
        success: function(response) {
          var module = app.modules.user;
          console.log(response);
          if (response.status) {
            module.config = JSON.parse(response.data.config);
            module.additional_data = response.data.additional_data;
            module.config.watched_tutorial = false;
            module.functions.apply_config();
            app.notify({
              type: "logged_in"
            });
            app.notify({
            ...
            app.modules.notifications.functions.show("Log in error", "cross");
          }
        }
      });
    },
